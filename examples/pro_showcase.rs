...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    render_frame(
        &mut renderer,
        &shapes_scene,
        "docs/images/shapes_showcase.png",
    )
    .await?;
...
fn build_main_showcase() -> SceneGraph {
    ...
    for i in 0..6 {
        ...
        let c = s
            .create_node_with_transform(format!("C{}", i), Transform::from_translation(x, y1, 0.0));
        s.get_node_mut(c)
            .unwrap()
...
fn build_shapes_showcase() -> SceneGraph {
    ...
    // Circle
    let c1 =
        s.create_node_with_transform("Circle".into(), Transform::from_translation(-0.6, 0.3, 0.0));
    s.get_node_mut(c1)
        .unwrap()
        .set_renderable(Renderable::Circle {
            radius: 0.2,
    ...
    let c2 =
        s.create_node_with_transform("Square".into(), Transform::from_translation(-0.2, 0.3, 0.0));
    s.get_node_mut(c2)
        .unwrap()
        .set_renderable(Renderable::Circle {
            radius: 0.2,
    ...
    // Triangle
    let c3 = s.create_node_with_transform(
        "Triangle".into(),
        Transform::from_translation(0.2, 0.3, 0.0),
    );
    s.get_node_mut(c3)
        .unwrap()
        .set_renderable(Renderable::Circle {
...
fn build_text_showcase(renderer: &mut ShapeRenderer) -> SceneGraph {
    ...
    // Title
    let t1 =
        s.create_node_with_transform("Title".into(), Transform::from_translation(0.0, 0.6, 0.0));
    s.get_node_mut(t1)
        .unwrap()
    ...
    // Subtitle
    let t2 = s.create_node_with_transform(
        "Subtitle".into(),
    ...
    // Stats
    let s1 =
        s.create_node_with_transform("Stats".into(), Transform::from_translation(0.0, -0.6, 0.0));
    s.get_node_mut(s1)
        .unwrap()
        .set_renderable(Renderable::Text {
...
async fn render_frame(
    ...
    for (t, r, o) in scene.get_visible_renderables() {
        ...
        if let Some((rad, col)) = r.as_circle() {
            ...
            renderer.draw_line(*s, *e, a(*col), *th, offset, &mut pass);
        } else if let Some((s, e, col, th)) = r.as_arrow() {
            renderer.draw_arrow(*s, *e, a(*col), *th, offset, &mut pass);
        } else if let Some((txt, sz, col)) = r.as_text() {
            renderer.draw_text(txt, *sz, a(*col), offset, &mut pass);
        }
    }
