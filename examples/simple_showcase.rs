...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    save_frame(&renderer, &tex, WIDTH, HEIGHT, "docs/images/diomanim_showcase.png");
...
fn build_scene() -> SceneGraph {
    ...
    s.get_node_mut(st).unwrap().set_renderable(Renderable::Text {
    ...
    let perf = s.create_node_with_transform("Perf".into(), Transform::from_translation(0.0, -0.8, 0.0));
    s.get_node_mut(perf).unwrap().set_renderable(Renderable::Text {
