...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("╚═══════════════════════════════════════════════════════════════╝\n");
    println!(
        "📹 Rendering {}x{} @ {} FPS for {}s",
        WIDTH, HEIGHT, FPS as u32, DURATION
    );
    ...
    for frame_count in 0..total_frames {
        ...
        save_frame(
            ...
            WIDTH,
            HEIGHT,
            &format!("{}/frame_{:04}.png", frames_dir, frame_count),
        );
        ...
        if (frame_count + 1) % 30 == 0 || frame_count + 1 == total_frames {
            println!(
                ...
                frame_count + 1,
                total_frames,
                (frame_count + 1) as f32 / total_frames as f32 * 100.0
            );
        }
    }
    ...
    println!(
        "\n✅ Rendered in {:.1}s ({:.1} FPS)",
        elapsed,
        total_frames as f32 / elapsed
    );

    match export_video(
        frames_dir,
        "output/gradient_descent_demo.mp4",
        WIDTH,
        HEIGHT,
        FPS as u32,
    ) {
...
fn build_scene() -> SceneGraph {
    ...
    for (i, &x) in steps.iter().enumerate() {
        ...
        let p = s.create_node_with_transform(
            format!("P{}", i),
            Transform::from_translation(px, py, 0.0),
        );
        let t = i as f32 / 3.0;
        s.get_node_mut(p)
            .unwrap()
            .set_renderable(Renderable::Circle {
                radius: 0.04,
                color: Color::new(0.3 + t * 0.6, 0.5 + t * 0.5, 1.0 - t * 0.4),
            });
        s.get_node_mut(p)
            .unwrap()
            .add_animation(AnimationInstance::new(
                effects::grow_from_center(0.3),
                TimeValue::new(1.5 + i as f32 * 0.6),
            ));
        ...
        if i < 3 {
            ...
            s.get_node_mut(a)
                ...
                .add_animation(AnimationInstance::new(
                    effects::create(0.3),
                    TimeValue::new(1.7 + i as f32 * 0.6),
                ));
        }
    }
    ...
    for i in 0..15 {
        ...
        s.get_node_mut(l)
            ...
            .add_animation(AnimationInstance::new(
                effects::create(0.15),
                TimeValue::new(0.8 + i as f32 * 0.03),
            ));
    }
...
fn save_frame(renderer: &ShapeRenderer, texture: &wgpu::Texture, w: u32, h: u32, filename: &str) {
    ...
    if p != u {
        ...
        for row in 0..h {
            ud.extend_from_slice(&data[(row * p) as usize..(row * p + u) as usize]);
        }
