...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nRendering {} frames at {} FPS...", (DURATION * FPS) as u32, FPS);
    ...
    // Export to video
    println!("\n");
    match export_video(frames_dir, "output/gradient_descent.mp4", WIDTH, HEIGHT, FPS as u32) {
        Ok(()) => {
...
fn build_gradient_descent_scene() -> SceneGraph {
    ...
    // Title
    let title_id = scene.create_node_with_transform("Title".to_string(), Transform::from_translation(0.0, 0.85, 0.0));
    scene.get_node_mut(title_id).unwrap().set_renderable(Renderable::Text {
        content: "Gradient Descent Optimization".to_string(),
        font_size: 56.0,
        color: Color::new(1.0, 1.0, 1.0),
    });
    scene.get_node_mut(title_id).unwrap().add_animation(AnimationInstance::new(effects::fade_in(1.0), TimeValue::new(0.0)));

    // Equations
    let eq1_id = scene.create_node_with_transform("Eq1".to_string(), Transform::from_translation(-0.5, 0.6, 0.0));
    scene.get_node_mut(eq1_id).unwrap().set_renderable(Renderable::Math {
        latex: "\\nabla f = 2x".to_string(),
        font_size: 42.0,
        color: Color::new(0.3, 0.8, 1.0),
    });
    scene.get_node_mut(eq1_id).unwrap().add_animation(AnimationInstance::new(effects::fade_in(0.8), TimeValue::new(1.0)));

    let eq2_id = scene.create_node_with_transform("Eq2".to_string(), Transform::from_translation(-0.6, 0.4, 0.0));
    scene.get_node_mut(eq2_id).unwrap().set_renderable(Renderable::Math {
        latex: "x_{t+1} = x_t - \\alpha \\nabla f".to_string(),
        font_size: 40.0,
        color: Color::new(0.3, 1.0, 0.6),
    });
    scene.get_node_mut(eq2_id).unwrap().add_animation(AnimationInstance::new(effects::fade_in(0.8), TimeValue::new(1.5)));

    let eq3_id = scene.create_node_with_transform("Eq3".to_string(), Transform::from_translation(-0.5, 0.2, 0.0));
    scene.get_node_mut(eq3_id).unwrap().set_renderable(Renderable::Math {
        latex: "\\alpha = 0.1".to_string(),
        font_size: 38.0,
        color: Color::new(1.0, 0.8, 0.3),
    });
    scene.get_node_mut(eq3_id).unwrap().add_animation(AnimationInstance::new(effects::fade_in(0.8), TimeValue::new(2.0)));
    ...
    for (i, &x) in steps.iter().enumerate() {
        ...
        let point_id = scene.create_node_with_transform(format!("Point{}", i), Transform::from_translation(pos_x, pos_y, 0.0));
        let t = i as f32 / (steps.len() - 1) as f32;
        let color = Color::new(0.3 + t * 0.6, 0.5 + t * 0.5, 1.0 - t * 0.4);
        
        scene.get_node_mut(point_id).unwrap().set_renderable(Renderable::Circle { radius: 0.04, color });
        scene.get_node_mut(point_id).unwrap().add_animation(AnimationInstance::new(effects::grow_from_center(0.5), TimeValue::new(2.5 + i as f32 * 0.8)));

        let label_id = scene.create_node_with_transform(format!("Label{}", i), Transform::from_translation(pos_x, pos_y - 0.12, 0.0));
        scene.get_node_mut(label_id).unwrap().set_renderable(Renderable::Text {
            content: format!("{}", i),
            font_size: 24.0,
            color: Color::new(0.7, 0.7, 0.7),
        });
        scene.get_node_mut(label_id).unwrap().add_animation(AnimationInstance::new(effects::fade_in(0.3), TimeValue::new(2.7 + i as f32 * 0.8)));
    }
    ...
    for i in 0..(steps.len() - 1) {
        ...
        scene.get_node_mut(arrow_id).unwrap().add_animation(AnimationInstance::new(effects::create(0.4), TimeValue::new(2.8 + i as f32 * 0.8)));
    }
    ...
    for i in 0..20 {
        ...
        scene.get_node_mut(line_id).unwrap().add_animation(AnimationInstance::new(effects::create(0.2), TimeValue::new(1.5 + i as f32 * 0.03)));
    }

    // Labels
    let xlabel_id = scene.create_node_with_transform("XLabel".to_string(), Transform::from_translation(0.5, -0.65, 0.0));
    scene.get_node_mut(xlabel_id).unwrap().set_renderable(Renderable::Text {
        content: "x".to_string(),
        font_size: 32.0,
        color: Color::new(0.8, 0.8, 0.8),
    });

    let ylabel_id = scene.create_node_with_transform("YLabel".to_string(), Transform::from_translation(-0.6, 0.0, 0.0));
    scene.get_node_mut(ylabel_id).unwrap().set_renderable(Renderable::Math {
...
fn save_frame_to_png(renderer: &ShapeRenderer, texture: &wgpu::Texture, width: u32, height: u32, filename: &str) {
    ...
    let mut encoder = renderer.get_device().create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Copy Encoder") });
